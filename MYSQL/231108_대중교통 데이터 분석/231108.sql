...
-- 연습 1
-- 승차 및 하차 인원수가 가장 적고 많은 역 조회
-- 승차인원 가장 많은 역과 가장 낮은 역
-- 하차인원 가장 많은 역과 가장 낮은 역
...
-- 퇴근 시간에 하차 인원이 가장 많은 순으로 데이터 조회
SELECT
    A.STATN_NO,
    A.STATN_NM,
    A.HO_LN,
...
-- 출근 시간때에 승차 인원이 가장 많으 순으로 데이터 조회
SELECT
    A.STATN_NO,
    A.STATN_NM,
    A.HO_LN,
...
-- 23시 이후에 사람들이 가장 많이 승차하는 역 구하기 => 막차 타는 사람들
SELECT
    A.STATN_NO,
    A.STATN_NM,
    A.HO_LN,
...
FROM TB_PBTRNSP A
WHERE A.STD_MT = '202304'
  AND (A.BEGIN_TIME, A.END_TIME) IN (
    ('2300','2400'),
    ('0000','0100'),
    ('0100','0200'),
    ('0200','0300'),   
...
-- 각 호선 별 승하차 인원수가 가장 많은 역 구하기 (숙제)
